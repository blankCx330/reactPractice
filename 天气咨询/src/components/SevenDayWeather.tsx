...
export default function SevenDayWeather() {
  ...
  const weatherList = () => {
    ...
    if (weatherDailyIsLoading) {
      return <div className="seven-day-weather-content-item bg-[rgba(255,138,43,0.5)] hover:bg-[rgba(255,138,43,0.8)] text-black/90 dark:bg-white/10 dark:text-white/90 dark:hover:bg-white/10">加载中...</div>
    }
    return weatherDaily.daily.map(data => (
      <div key={data.fxDate} className="seven-day-weather-content-item bg-[rgba(255,138,43,0.5)] hover:bg-[rgba(255,138,43,0.8)] text-black/90 dark:bg-white/10 dark:text-white/90 dark:hover:bg-white/10">
  ...
  return (
    <div className="seven-day-weather transition-all bg-[rgb(229,231,120)] dark:bg-[rgb(60,60,60)]">
      ...
      <div className="seven-day-weather-content">{weatherList()}</div>
    </div>
  )
}
